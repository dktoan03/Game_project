...
{
    g_background = SDL_General::LoadImage("background.png", ren);
...
bool SDL_General::Do_main_bullet_overlap_enemy(SDL_Rect mainbullet, SDL_Rect enemy) {
    if ( mainbullet.y - enemy.y >=0 && mainbullet.y - enemy.y <= 74 && mainbullet.x - enemy.x >= 36 &&  mainbullet.x - enemy.x <= (80 - 11) ) { //11 là chiều ngang đạn main
        return true;
    }
