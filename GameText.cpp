...
string GameText::ConvertIntToString2(){
    int v = value_ ;
    string stringText ;
    if(v<=0) {
        stringText = '0' ;
        stringText="LIVE :"+stringText;
        return stringText ;
    }
    while(v){
        stringText =(char)(v%10+48) + stringText ;
    ...
    if(stringText!="1") stringText="LIVES: "+stringText;
    else stringText="LIVE :"+stringText;
    return stringText ;
}
 void GameText::ShowNum2 (TTF_Font *gFont, SDL_Color textColor, SDL_Renderer *gRenderer  )
{
    if ( loadFromRenderedText(gFont, ConvertIntToString2() , textColor,gRenderer ) )
    {
        SDL_RenderCopy(gRenderer,p_object_,NULL,&rect_);
        SDL_RenderPresent(gRenderer);
    }
}
string GameText::ConvertIntToString3(){
    int v = value_ ;
    string stringText ;
    if(v<=0) stringText = '0' ;
    while(v){
    ...
    stringText="YOUR FINAL SCORE: "+stringText;
    return stringText ;
}
 void GameText::ShowNum3 (TTF_Font *gFont, SDL_Color textColor, SDL_Renderer *gRenderer  )
{
    if ( loadFromRenderedText(gFont, ConvertIntToString3() , textColor,gRenderer ) )
    {
        SDL_RenderCopy(gRenderer,p_object_,NULL,&rect_);
        SDL_RenderPresent(gRenderer);
    }
